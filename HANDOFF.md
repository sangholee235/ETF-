...
> 상세한 발견·수정 경위, 고민과 결정 과정은 **`DEVLOG.md`** 참고.
...
- **⚠️ 미해결: 시장가 매수 "매수증거금 부족"**: 지금은 `_MARKET_ORDER_MARGIN_BUFFER = 1.3`으로 보수적 수량 계산 중(추측값). 부작용: 주문 쪼개짐. "현재가+0.5% 지정가" 전환 안이 논의됐으나 **사용자 확정 전 구현 중단됨** — 다음 세션 최우선 안건.
  - **부작용 사례**: 일일한도 60,000원 중 27,000원 매수 완료(남은예산 33,000원) 상태에서 29,000원대 종목을 사려 했는데, `29,000×1.3≈37,700 > 33,000`이라 SKIP됨. 실제 가격×수량은 예산 안에 들어오는데도 버퍼 때문에 막힌 것 — 근데 SKIP 사유가 "입금이 필요합니다"라 사용자가 "예수금도 한도도 안 모자라는데 왜?"라고 혼란스러워함. **1차 조치**: 근본(버퍼값 자체) 수정은 보류하고, 메시지만 "시장가 증거금 버퍼(가격의 1.3배, 추정치) 때문"이라고 원인을 명시하도록 수정(`runner.py`, `routers/bot.py`) — "입금이 필요하다"는 오해를 주지 않게. 버퍼 자체를 낮추거나 지정가 방식으로 바꿀지는 여전히 사용자 확정 필요.
- **조회 라우터(account/market/orders)가 키움의 RuntimeError를 안 잡던 버그**: `executor.py`에서 이미 한 번 겪었던 것과 같은 종류(§ "주문 실패가 실행기록에 안 남던 버그") — 키움은 실패 시 `TossApiError`가 아니라 `RuntimeError`를 던지는데(`kiwoom.py`의 `_request()`/`_get_token()`), `account.py`·`market.py`·`orders.py`의 **모든** 엔드포인트가 `except TossApiError`만 잡고 있었음. 그래서 키움 쪽 실패(토큰 문제·IP 인증·일시오류 등) 시 FastAPI가 처리 안 된 500을 던지고, 프론트의 `.catch(() => {})`가 조용히 삼켜서 **"에러 메시지도 없고 로딩도 안 끝난 것도 아닌데 그냥 `-`만 뜨는"** 증상으로 나타남(사용자가 "키움계좌 조회가 왜 안되지"로 발견). **수정**: `deps.py`에 `to_http_any()` 추가(TossApiError/RuntimeError 공통 변환, RuntimeError는 502)하고 세 라우터 전체의 except 절을 `(TossApiError, RuntimeError)`로 확장. `orders.py`의 `cancel_order`는 원래 `RuntimeError`를 400("취소를 지원하지 않습니다")으로 더 친절하게 잡던 별도 except가 있었는데, 일괄 치환 과정에서 겹쳐 죽은 코드가 될 뻔해 그 엔드포인트만 원래 로직 보존하도록 별도 처리.
